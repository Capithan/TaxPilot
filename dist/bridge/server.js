...
import { findNearbyOffices } from '../services/offices.js';
...
function buildPluginManifest(baseUrl) {
    return {
        schema_version: 'v1',
    ...
    try {
        switch (name) {
            ...
            case 'show_booking_form': {
                const bfClientId = args.clientId;
                const bfTaxProId = args.taxProId;
                    ...
                    components: [
                        ...
                        ...(bfTaxPro ? [{
                                ...
                                fields: [
                                    ...
                                    { label: 'Specializations', value: bfTaxPro.specializations.map((s) => s.replace(/_/g, ' ')).join(', '), icon: '📌' },
                                ],
                            }] : []),
                        {
                            type: 'form_group',
            ...
            case 'create_appointment': {
                ...
                const appointmentType = (formData?.appointmentType || args.type || 'virtual');
                const appointment = createAppointment(clientId, taxProId, scheduledAt, appointmentType);
            ...
            case 'find_nearby_offices': {
                const officeQuery = args.query;
                const officeClientId = args.clientId;
