...
import { createDocumentReminder, getClientReminders, formatRemindersForDisplay, } from './services/reminders.js';
        ...
        try {
            switch (name) {
                ...
                case 'create_document_reminders': {
                    ...
                    return {
                        content: [{ type: 'text', text: reminders.map((r) => `- ${r.message}`).join('\n') }],
                    };
                }
                ...
                case 'get_tax_pro_recommendations': {
                    const recs = getTaxProRecommendations(args?.clientId);
                    return {
                        content: [{ type: 'text', text: recs }],
                    };
                }
                case 'create_appointment': {
                    const appointment = createAppointment(args?.clientId, args?.taxProId, new Date(args?.scheduledAt), args?.type);
                    return {
