...
function renderInfoCard(c) {
    ...
    const actions = (c.actions || []).map(a => renderComponent(a)).join('');
...
function renderTaxProCard(c) {
    ...
    const specs = (pro.specializations || []).map(s => `<span class="tp-badge tp-badge--info">${esc(s)}</span>`).join(' ');
...
function renderCarousel(c) {
    const items = (c.items || []).map(item => `<div class="tp-carousel-item">${renderComponent(item)}</div>`).join('');
...
function renderComponent(c) {
    ...
    switch (kind) {
        ...
        case 'TextBlock':
        case 'text_block': return renderTextBlock(c);
        case 'Divider':
        case 'divider': return renderDivider(c);
        case 'StepProgress':
        case 'step_progress': return renderStepProgress(c);
        case 'ProgressBar':
        case 'progress_bar': return renderProgressBar(c);
        case 'StatusBadge':
        case 'status_badge': return renderStatusBadge(c);
        case 'FormGroup':
        case 'form_group': return renderFormGroup(c);
        case 'SelectionCard':
        case 'selection_card': return renderSelectionCard(c);
        case 'MultiSelectCard':
        case 'multi_select': return renderMultiSelect(c);
        case 'InfoCard':
        case 'info_card': return renderInfoCard(c);
        case 'Button':
        case 'button': return renderButton(c);
        case 'Checklist':
        case 'checklist': return renderChecklist(c);
        case 'TaxProCard':
        case 'tax_pro_card': return renderTaxProCard(c);
        case 'AppointmentSummaryCard':
        case 'appointment_summary': return renderAppointmentSummary(c);
        case 'Carousel':
        case 'carousel': return renderCarousel(c);
...
function renderUIResponseBody(resp) {
    const parts = [];
    if (resp.title)
        parts.push(`<div class="tp-text tp-text--heading">${esc(resp.title)}</div>`);
    if (resp.subtitle)
        parts.push(`<div class="tp-text tp-text--subheading">${esc(resp.subtitle)}</div>`);
    // Banner
    const banner = resp.banner;
    if (banner?.text)
        parts.push(renderBanner(banner));
    // Progress
    ...
    // Message
    if (resp.message)
        parts.push(`<div class="tp-text tp-text--body">${esc(resp.message)}</div>`);
    // Actions
 ...
 */
export function toHtmlWidget(resp) {
    if (!resp)
        return null;
...
//# sourceMappingURL=toHtmlWidget.js.map