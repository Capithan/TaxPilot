...
function buildStepProgress(currentStep, completedSteps) {
    return {
        ...
        steps: ALL_STEP_IDS.map(id => ({
            ...
            status: completedSteps.includes(id) ? 'done'
...
function buildProgressBar(completedSteps) {
    return {
        type: 'progress_bar',
...
function toolCallAction(tool, parameters = {}) {
...
function personalInfoUI(sessionId) {
    return {
        type: 'form_group',
...
function filingStatusUI(sessionId) {
    return {
        type: 'selection_card',
...
function dependentsUI(sessionId) {
    return [
        {
            type: 'selection_card',
...
function dependentFormUI(sessionId) {
    return {
        type: 'form_group',
...
function employmentUI(sessionId) {
    return {
        type: 'form_group',
...
function incomeTypesUI(sessionId) {
    return {
        type: 'multi_select',
...
function deductionsUI(sessionId) {
    return {
        type: 'multi_select',
...
function specialSituationsUI(sessionId) {
    return {
        type: 'multi_select',
...
function documentUploadUI(sessionId) {
    return {
        type: 'selection_card',
 ...
 */
function buildStructuredResponse(screen, toolName, currentStep, completedSteps, stepComponents, data, opts) {
    const components = [];
...
function getStepComponents(step, sessionId) {
    switch (step) {
        case 'personal_info':
...
export function formatIntakeStart(result, flowStage) {
    const sessionId = result.session.id;
    const currentStep = result.currentStep;
    const completedSteps = [];
    const stepComponents = getStepComponents(currentStep, sessionId);
    return buildStructuredResponse('intake', 'start_intake', currentStep, completedSteps, [
        // Welcome banner
...
export function formatIntakeResponse(result, sessionId, progressInfo) {
    ...
    if (result.intakeCompleted) {
        return {
            ...
            components: [
                ...
                buildStepProgress('complete', ALL_STEP_IDS),
    ...
    const bannerOpt = result.stepCompleted
        ? { banner: { type: 'banner', text: `✅ Step completed! Now let's work on ${STEP_LABELS[currentStep] || currentStep}.`, variant: 'success' } }
        : undefined;
    return buildStructuredResponse('intake', 'process_intake_response', currentStep, completedSteps, stepComponents, {
        sessionId,
        clientId: result.client?.id,
        currentStep,
        nextQuestion: result.nextQuestion,
...
function buildReviewUI(sessionId, client, completedSteps) {
    const fields = [
    ...
    const specials = [];
    if (client.hasCrypto)
        specials.push('Cryptocurrency');
    if (client.hasForeignAccounts)
        specials.push('Foreign Accounts');
    if (client.hasRentalProperty)
        specials.push('Rental Property');
    if (client.hasBusinessIncome)
        specials.push('Business Income');
...
export function formatIntakeProgress(progress, sessionId) {
    ...
    const components = [
        ...
        buildStepProgress(progress.currentStep, progress.completedSteps),
...
export function formatClientSummary(client, summaryText) {
    ...
    components.push({ type: 'text_block', text: 'Please review this information carefully. Is everything correct?', style: 'body' }, {
        type: 'button', label: '✅ Confirm — Everything is Correct', variant: 'primary', icon: '✅',
