...
export function calculateComplexityScore(client) {
    let score = 0;
    // Filing status
    ...
    // Special situations
    if (client.hasCrypto)
        score += COMPLEXITY_WEIGHTS.specialSituations.crypto;
    if (client.hasForeignAccounts)
        score += COMPLEXITY_WEIGHTS.specialSituations.foreignAccounts;
    if (client.hasRentalProperty)
        score += COMPLEXITY_WEIGHTS.specialSituations.rentalProperty;
    if (client.hasBusinessIncome)
        score += COMPLEXITY_WEIGHTS.specialSituations.businessIncome;
    // Dependents
    const dependentScore = Math.min(client.dependents.length * COMPLEXITY_WEIGHTS.dependentsBase, COMPLEXITY_WEIGHTS.dependentsMax);
    score += dependentScore;
    // Cap the score at 100
    return Math.min(score, 100);
}
export function getComplexityLevel(score) {
    if (score <= COMPLEXITY_THRESHOLDS.simple.max)
        return 'simple';
    if (score <= COMPLEXITY_THRESHOLDS.moderate.max)
        return 'moderate';
    if (score <= COMPLEXITY_THRESHOLDS.complex.max)
        return 'complex';
    return 'expert';
}
export function getRequiredSpecializations(client) {
    const specs = new Set();
    // Base specialization
    ...
    // Investments
    if (client.incomeTypes.includes('investment_income') ||
...
export function findBestTaxPro(client) {
    ...
    const scoredPros = availablePros.map((pro) => {
        ...
        const proComplexityOrder = ['simple', 'moderate', 'complex', 'expert'];
...
function formatRoutingReason(client, taxPro, complexity, requiredSpecs) {
...
export function routeClientToTaxPro(clientId) {
...
export function createAppointment(clientId, taxProId, scheduledAt, type = 'virtual') {
    const client = db.getClient(clientId);
    if (!client) {
...
export function getAppointmentEstimate(clientId) {
    const client = db.getClient(clientId);
    if (!client) {
        throw new Error('Client not found');
    }
...
export function getTaxProRecommendations(clientId) {
    const client = db.getClient(clientId);
    if (!client) {
