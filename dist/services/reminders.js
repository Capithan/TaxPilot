...
export function createDocumentReminder(clientId, appointmentId, documents) {
...
function generatePersonalizedMessage(client, doc) {
...
export function createAppointmentReminder(clientId, appointmentId, reminderType, scheduledFor) {
    const client = db.getClient(clientId);
...
export function createBatchDocumentReminder(clientId, appointmentId) {
    const client = db.getClient(clientId);
    if (!client)
        return null;
...
export function getClientReminders(clientId) {
    return db.getRemindersByClient(clientId);
}
export function getPendingReminders() {
    return db.getPendingReminders();
}
export function sendReminder(reminderId) {
...
export function scheduleAppointmentReminders(appointment) {
    const reminders = [];
    ...
    if (reminder24h > new Date()) {
        reminders.push(createAppointmentReminder(appointment.clientId, appointment.id, 'appointment_reminder_24h', reminder24h));
    }
    // 1-hour reminder
    const reminder1h = new Date(appointmentTime);
    reminder1h.setHours(reminder1h.getHours() - 1);
    if (reminder1h > new Date()) {
        reminders.push(createAppointmentReminder(appointment.clientId, appointment.id, 'appointment_reminder_1h', reminder1h));
    }
