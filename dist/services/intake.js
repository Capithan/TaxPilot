...
export function startIntakeSession(clientId) {
...
function updateClientFromResponse(client, step, answer) {
    const lowerAnswer = answer.toLowerCase();
    switch (step) {
        case 'personal_info':
...
export function getIntakeProgress(sessionId) {
    const session = db.getSession(sessionId);
    if (!session)
        return null;
    const currentIndex = INTAKE_STEPS.indexOf(session.currentStep);
...
export function getIntakeSummary(clientId) {
    ...
    if (client.hasCrypto)
        summary += `  - Cryptocurrency transactions\n`;
    if (client.hasForeignAccounts)
        summary += `  - Foreign accounts/income\n`;
    if (client.hasRentalProperty)
        summary += `  - Rental property\n`;
