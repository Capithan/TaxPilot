 ...
 */
const EXTERNAL_RESOURCE_DOMAINS: string[] = [
...
const EXTERNAL_CONNECT_DOMAINS: string[] = [
 ...
 */
function getCspDomains(): { resourceDomains: string[]; connectDomains: string[] } {
 ...
 */
function getToolMeta(): Record<string, unknown> {
  return {
    ui: {
 ...
 */
function getInvocationMeta(): Record<string, unknown> {
  return {
    ui: {
      resourceUri: getWidgetResourceUri(),
      csp: getCspDomains(),
 ...
 */
function toMcpContent(uiResp: UIResponse | Record<string, unknown>): {
  ...
  return {
    content: [{ type: 'text' as const, text: `TaxPilot: ${screen}` }],
  ...
  try {
    switch (name) {
      ...
      case 'advance_conversation_flow': {
        const clientId = args?.clientId as string;
        const stageData = args?.stageData as Record<string, unknown> | undefined;
      ...
      case 'set_scheduling_preferences': {
        const clientId = args?.clientId as string;
        const preferences = {
          preferredDates: args?.preferredDates as string[] || [],
          preferredTimes: args?.preferredTimes as string[] || [],
          appointmentType: (args?.appointmentType as 'virtual' | 'in_person') || 'virtual',
        };
      ...
      case 'select_tax_professional': {
        const clientId = args?.clientId as string;
        const taxProId = args?.taxProId as string;
