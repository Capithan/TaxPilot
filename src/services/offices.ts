 ...
 */
export function findNearbyOffices(query?: string, limit: number = 6): OfficeLocation[] {
  let results = [...OFFICES];
  ...
  if (query) {
    ...
    if (results.length === 0) results = [...OFFICES];
  }
 ...
 */
export function getOffice(officeId: string): OfficeLocation | undefined {
