 ...
 */
export function processStructuredIntakeResponse(
  sessionId: string,
  step: string,
...
function applyStructuredData(
  ...
  switch (step) {
    case 'personal_info': {
      if (formData) {
        if (formData.firstName) client.firstName = formData.firstName;
        if (formData.lastName) client.lastName = formData.lastName;
        if (formData.email) client.email = formData.email;
        if (formData.phone) client.phone = formData.phone;
        if (formData.dateOfBirth) client.dateOfBirth = formData.dateOfBirth;
        if (formData.address) {
    ...
    case 'filing_status': {
      if (selection) {
        client.filingStatus = selection as FilingStatus;
      }
      break;
    }
    ...
    case 'employment': {
      if (formData) {
        const emp: EmploymentInfo = {
          employerName: formData.employerName || '',
          jobTitle: formData.jobTitle || '',
          isCurrentJob: true,
          startDate: '',
          incomeType: (formData.incomeType as 'W2' | '1099' | 'both') || 'W2',
    ...
    case 'income_types': {
      if (selections) {
        const incomeTypes = selections as IncomeType[];
        client.incomeTypes = [...new Set([...client.incomeTypes, ...incomeTypes])];
        if (selections.includes('crypto_income')) client.hasCrypto = true;
        if (selections.includes('rental_income')) client.hasRentalProperty = true;
        if (selections.includes('foreign_income')) client.hasForeignAccounts = true;
      }
      break;
    }
    ...
    case 'deductions': {
      if (selections) {
        const deductions = selections as DeductionType[];
        client.deductions = [...new Set([...client.deductions, ...deductions])];
        if (selections.includes('business_expenses')) client.hasBusinessIncome = true;
      }
      break;
    }
    ...
    case 'special_situations': {
      if (selections) {
        if (selections.includes('crypto')) {
          ...
          if (!client.incomeTypes.includes('crypto_income')) client.incomeTypes.push('crypto_income');
        }
        if (selections.includes('foreign_accounts')) {
          client.hasForeignAccounts = true;
          if (!client.incomeTypes.includes('foreign_income')) client.incomeTypes.push('foreign_income');
        }
