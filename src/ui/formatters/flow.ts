...
const STAGE_LABELS: Record<ConversationStage, { label: string; icon: string }> = {
...
function stageBadge(
  ...
  if (completedStages.includes(stage)) return { text: 'Done', variant: 'success', icon: '✅' };
  if (stage === currentStage) return { text: 'Current', variant: 'info', icon: '🔵' };
...
export function formatFlowStatus(
  ...
  const builder = ui.response('flow_status', 'get_conversation_flow')
    ...
    .progress(flowResult.progress.current, flowResult.progress.total,
      `${flowResult.progress.percentage}% complete`);
...
export function formatFlowAdvanced(
  flowResult: FlowActionResult,
  instructions: string,
  clientId: string,
): UIResponse {
...
export function formatSummaryConfirmed(
  flowResult: FlowActionResult,
  instructions: string,
  clientId: string,
): UIResponse {
...
export function formatSchedulingPreferences(
  preferences: {
...
export function formatTaxProSelected(
  taxProName: string,
  taxProId: string,
  instructions: string,
  clientId: string,
