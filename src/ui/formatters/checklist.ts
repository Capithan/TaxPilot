...
import type { UIResponse, UIListItem } from '../types.js';
...
function docToListItem(doc: DocumentItem, clientId: string): UIListItem {
  return {
    text: doc.name,
...
export function formatDocumentChecklist(
  ...
  toolName: 'generate_document_checklist' | 'get_document_checklist' = 'generate_document_checklist',
): UIResponse {
  ...
  builder.nextTools(
    collected < total ? 'mark_document_collected' : 'route_to_tax_pro',
...
export function formatDocumentCollected(result: {
  success: boolean;
...
export function formatPendingDocuments(
