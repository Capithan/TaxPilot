...
// ─── Component Renderers (mirror the client-side WidgetRenderer) ─────────────
...
function renderTextBlock(c: Record<string, unknown>): string {
  const cls: Record<string, string> = { heading: 'tp-text--heading', subheading: 'tp-text--subheading', caption: 'tp-text--caption' };
  return `<div class="tp-text ${cls[c.style as string] || 'tp-text--body'}">${esc(c.text)}</div>`;
}
...
function renderStepProgress(c: Record<string, unknown>): string {
  const steps = (c.steps || []) as Array<{ label: string; status: string; icon?: string }>;
  ...
  return `<div class="tp-steps">${parts}</div>`;
}

function renderProgressBar(c: Record<string, unknown>): string {
  const current = (c.current as number) || (c.value as number) || 0;
  const total = (c.total as number) || (c.max as number) || 100;
...
function renderStatusBadge(c: Record<string, unknown>): string {
...
function renderFormGroup(c: Record<string, unknown>): string {
  ...
  const fields = ((c.fields || []) as Array<Record<string, unknown>>).map(f => {
    ...
    switch (fType) {
      case 'select': {
        ...
        input = `<select class="tp-input tp-select"><option value="" disabled selected>${esc(f.placeholder || 'Select...')}</option>${opts.map(o => `<option value="${esc(o.value)}">${esc(o.label)}</option>`).join('')}</select>`;
        break;
      }
      case 'textarea':
  ...
  return `<div class="tp-form-group">${title}${subtitle}<div class="tp-form-fields">${fields}</div><div class="tp-form-note" style="font-size:12px;color:#888;text-align:center;margin-top:8px">💬 Answer in the chat to continue</div></div>`;
}

function renderSelectionCard(c: Record<string, unknown>): string {
  const title = c.title ? `<div class="tp-sel-title">${esc(c.title)}</div>` : '';
  const options = ((c.options || []) as Array<Record<string, unknown>>).map(opt => {
  ...
  return `<div class="tp-sel">${title}<div class="tp-sel-options">${options}</div><div class="tp-form-note" style="font-size:12px;color:#888;text-align:center;margin-top:8px">💬 Tell me your choice in the chat</div></div>`;
}

function renderMultiSelect(c: Record<string, unknown>): string {
  ...
  return `<div class="tp-multisel">${title}${subtitle}<div class="tp-multisel-options">${options}</div><div class="tp-form-note" style="font-size:12px;color:#888;text-align:center;margin-top:8px">💬 Tell me your selections in the chat</div></div>`;
}

function renderInfoCard(c: Record<string, unknown>): string {
  const badgeObj = c.badge as { text?: string; variant?: string } | undefined;
  const badge = badgeObj ? `<span class="tp-badge tp-badge--${badgeObj.variant || 'info'}">${esc(badgeObj.text)}</span>` : '';
  ...
  const actions = ((c.actions || []) as Array<Record<string, unknown>>).map(a => renderComponent(a)).join('');
...
function renderButton(c: Record<string, unknown>): string {
  const variantCls: Record<string, string> = { danger: 'tp-btn--danger', secondary: 'tp-btn--secondary', ghost: 'tp-btn--ghost', success: 'tp-btn--primary' };
  const cls = variantCls[c.variant as string] || 'tp-btn--primary';
...
function renderChecklist(c: Record<string, unknown>): string {
  const icon = c.icon ? `<span class="tp-checklist-icon">${c.icon}</span>` : '';
  const counterObj = c.counter as { done?: number; total?: number } | undefined;
  const counter = counterObj ? `<span class="tp-checklist-counter">${counterObj.done}/${counterObj.total}</span>` : '';
  const items = ((c.items || []) as Array<Record<string, unknown>>).map(item => {
...
function renderTaxProCard(c: Record<string, unknown>): string {
  const pro = (c.taxPro || {}) as Record<string, unknown>;
  ...
  const rating = pro.rating as number || 0;
  const stars = '★'.repeat(Math.floor(rating)) + (rating % 1 >= 0.5 ? '½' : '');
  const specs = ((pro.specializations || []) as string[]).map(s => `<span class="tp-badge tp-badge--info">${esc(s)}</span>`).join(' ');
  const avail = pro.availability ? `<div class="tp-pro-avail">📅 ${esc(pro.availability)}</div>` : '';
  const initial = ((pro.name as string) || 'T').charAt(0);
...
function renderAppointmentSummary(c: Record<string, unknown>): string {
  const apt = (c.appointment || {}) as Record<string, unknown>;
  const pro = (c.taxPro || {}) as Record<string, unknown>;
  ...
  return `<div class="tp-appt-summary">${confId}
    <div class="tp-appt-sections">
      <div class="tp-appt-section"><div class="tp-appt-section-title">📅 Appointment Details</div>
        ${apt.date ? `<div class="tp-cardfield"><span class="tp-cardfield-icon">📆</span><span class="tp-cardfield-label">Date</span><span class="tp-cardfield-value">${esc(apt.date)}</span></div>` : ''}
        ${apt.time ? `<div class="tp-cardfield"><span class="tp-cardfield-icon">🕐</span><span class="tp-cardfield-label">Time</span><span class="tp-cardfield-value">${esc(apt.time)}</span></div>` : ''}
        ${apt.duration ? `<div class="tp-cardfield"><span class="tp-cardfield-icon">⏱</span><span class="tp-cardfield-label">Duration</span><span class="tp-cardfield-value">${esc(apt.duration)}</span></div>` : ''}
        ${apt.type ? `<div class="tp-cardfield"><span class="tp-cardfield-icon">${apt.type === 'Virtual' ? '💻' : '🏢'}</span><span class="tp-cardfield-label">Type</span><span class="tp-cardfield-value">${esc(apt.type)}</span></div>` : ''}
      ...
      <div class="tp-appt-section"><div class="tp-appt-section-title">👨‍💼 Tax Professional</div>
        <div class="tp-appt-pro-header"><div><div class="tp-appt-pro-name">${esc(pro.name)}</div><div class="tp-appt-pro-title">${esc(pro.title)}</div><div class="tp-appt-pro-rating"><span class="tp-stars">${stars}</span> ${rating}</div></div></div>
        <div class="tp-appt-pro-specs">${specs}</div>
      </div>
      ${client.name ? `<div class="tp-appt-section"><div class="tp-appt-section-title">👤 Client</div><div class="tp-cardfield"><span class="tp-cardfield-label">Name</span><span class="tp-cardfield-value">${esc(client.name)}</span></div></div>` : ''}
    </div>
  </div>`;
}

function renderCarousel(c: Record<string, unknown>): string {
  const items = ((c.items || []) as Array<Record<string, unknown>>).map(item =>
    `<div class="tp-carousel-item">${renderComponent(item)}</div>`
  ).join('');
...
function renderComponent(c: Record<string, unknown>): string {
  if (!c) return '';
  // Support both `component` key (client widget format) and `type` key (formatter output)
  const kind = (c.component || c.type) as string;
  if (!kind) return '';
  ...
  switch (kind) {
    ...
    default: return `<!-- unknown: ${esc(kind)} -->`;
  }
}

// ─── UIResponse (builder-format) renderer ────────────────────────────────────

function renderUIResponseBody(resp: Record<string, unknown>): string {
  const parts: string[] = [];
  ...
  // Banner
  const banner = resp.banner as Record<string, unknown> | undefined;
  if (banner?.text) parts.push(renderBanner(banner));
  ...
  if (cards?.length) {
    for (const card of cards) {
      const badgeObj = card.badge as { text?: string; variant?: string } | undefined;
      const badge = badgeObj ? `<span class="tp-badge tp-badge--${badgeObj.variant || 'info'}">${esc(badgeObj.text)}</span>` : '';
      const highlight = card.highlight ? `<div class="tp-card-highlight">${esc(card.highlight)}</div>` : '';
      const fields = ((card.fields || []) as Array<Record<string, unknown>>).map(f => {
        const icon = f.icon ? `<span class="tp-cardfield-icon">${f.icon}</span>` : '';
        return `<div class="tp-cardfield">${icon}<span class="tp-cardfield-label">${esc(f.label)}</span><span class="tp-cardfield-value">${esc(f.value)}</span></div>`;
      }).join('');

      const sections = ((card.sections || []) as Array<Record<string, unknown>>).map(sec => {
        const items = ((sec.items || []) as Array<Record<string, unknown>>).map(item => {
  ...
  // Standalone sections
  const sections = resp.sections as Array<Record<string, unknown>> | undefined;
  if (sections?.length) {
  ...
  // Actions
  const actions = resp.actions as Array<Record<string, unknown>> | undefined;
  if (actions?.length) {
    const btns = actions.map(a => renderButton(a)).join('');
...
export function toHtmlWidget(resp: Record<string, unknown> | null | undefined): string | null {
  ...
  if (resp.components && Array.isArray(resp.components)) {
    const components = resp.components as Array<Record<string, unknown>>;
    bodyHtml = components.map(c => renderComponent(c)).join('\n');
  }
  // UIResponse (builder format with type/title/cards)
  else if (resp.type || resp.title || resp.cards) {
    bodyHtml = renderUIResponseBody(resp);
  }
  // Unknown — skip
  else {
    return null;
...
/* Note for form/selection instructions */
